...
from finiteElement3D_t.question import Question, One_Dimensional_PoissonQuestion, Two_Dimensional_PoissonQuestion_t
...
class IntegrandFunction3D_t():
    def __init__(self,question:Question,trial,test):
        ...
        self._question = question
        self._rs_left=question.rs_left  # [(r,s,p,q,w,r),(r,s,p,q,w,r),(r,s,p,q,w,r)]
        self._pq_right= question.pq_right #[(q,w,r),(q,w,r),(q,w,r)]
        self.trial=trial
        self.test=test
        self.En=None
        self._alpha=0
    ...
    def A_F(self,x,y,z):
        ...
        '''
        f=0.0
        for r,s,p,q,w,e in self._rs_left:
            trial=self.trial(x,y,z, self.En, index=self._alpha, derivative_x=r,derivative_y=s,derivative_z=p)
            test=self.test(x,y,z, self.En, index=self._beta, derivative_x=q,derivative_y=w,derivative_z=e)
            f_l=self._question.C(x, y,z,self.t)* trial*test
            f = f+f_l
        return f
    ...
    def M_F(self,x,y,z):
        ...
        '''
        f=0.0
        trial=self.trial(x,y,z, self.En, index=self._alpha, derivative_x=0,derivative_y=0,derivative_z=0)
        test=self.test(x,y, z,self.En, index=self._beta, derivative_x=0,derivative_y=0,derivative_z=0)
        f_l=trial*test
        f = f+f_l
        return f
    ...
    def B_F(self,x,y,z):
        ...
        '''
        f = 0.0
        for q,w,e in self._pq_right:
            test = self.test(x,y,z,self.En, index=self._beta, derivative_x=q,derivative_y=w,derivative_z=e)
            f_l = self._question.F(x, y,z,self.t)* test
            f = f + f_l
        return f
...
def getIntegrandFunction(question:Question,trial,test):
    ...
    elif isinstance(question,Three_Dimensional_PoissonQuestion_t):
        return IntegrandFunction3D_t(question,trial,test)
    else:
