...
class PMatrixType(Enum):
    ...
    Three_Dimensional_Linear_Node=3
...
class TMatrixType(Enum):
    ...
    Three_Dimensional_Linear_Cell = 5
...
class PTMatrix():
    ...
    def _make_Three_Dimensional_Linear_Node(self,ranges, h):
            '''
    ...
    def _make_Three_Dimensional_Linear_Cell(self,N1, N2, N3):
        '''
        Global index of Three dimensional linear element nodes in mesh element according to the number of Mesh elements
        Corresponding to T and Tb
        :param N : Number of Mesh elements such as 2,2,2
        :return: Tb:[[ 0  1  3  4  9 10 12 13]
                    [ 1  2  4  5 10 11 13 14]
                    [ 4  5  7  8 13 14 16 17]
                    [ 3  4  6  7 12 13 15 16]
        ...
        '''
        N = N1 * N2 * N3
        Tb = np.zeros((8, N), dtype=int)
        for ze in range(N3):
            for ce in range(N1):
                for re in range(N2):
                    n = ze * (N1 * N2) + ce * N2 + re

                    j1 = re + ce * (N2 + 1) + ze * (N1 + 1) * (N2 + 1)
                    j2 = j1 + 1
                    j3 = j2 + (N2 + 1)
                    j4 = j3 - 1
    ...
    def getPTMatrix(self):
        ...
        if self._pMatrixType == PMatrixType.Three_Dimensional_Linear_Node:
            N1 = int((self._range[0][1] - self._range[0][0]) / self._h_Mesh[0])
            N2 = int((self._range[1][1] - self._range[1][0]) / self._h_Mesh[1])
        ...
        if self._tMatrixType == TMatrixType.Three_Dimensional_Linear_Cell:
            N1 = int((self._range[0][1] - self._range[0][0]) / self._h_Mesh[0])
            N2 = int((self._range[1][1] - self._range[1][0]) / self._h_Mesh[1])
            N3 = int((self._range[2][1] - self._range[2][0]) / self._h_Mesh[2])
            self.T = self._make_Three_Dimensional_Linear_Cell(N1,N2,N3)
    ...
    def getPbTbMatrix(self):
        ...
        if self._pbMatrixType == PMatrixType.Two_Dimensional_Linear_Node:
            ...
            self.Nb = N1 * N2 * N3
            self.Nbm = (N1 + 1) * (N2 + 1) * (N3 + 1)
            self.Pb = self._make_Three_Dimensional_Linear_Node(self._range, self._h_Finite)
